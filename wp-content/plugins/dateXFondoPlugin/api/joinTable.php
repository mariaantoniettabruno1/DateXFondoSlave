...
function esegui_modifica_ordinamento($params)
{
    if(isset($params["id"]) && $params["id"] > 0 ) {
        $success = \dateXFondoPlugin\FondoCompletoTableRepository::updateJoinedIndex($params["id"], $params["ordinamento"],$_GET['city']);
        if($success){
        ...
        $insert_id = \dateXFondoPlugin\FondoCompletoTableRepository::insertJoinedIndex($params["external_id"], $params["type"], $params["ordinamento"],$_GET['city']);
        if($insert_id){
