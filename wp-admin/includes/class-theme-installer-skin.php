...
class Theme_Installer_Skin extends WP_Upgrader_Skin {
	...
	private function do_overwrite() {
		...
		if ( ! empty( $new_theme_data['TemplateFondo'] ) ) {
			$is_invalid_parent = ! in_array( $new_theme_data['TemplateFondo'], array_keys( $all_themes ), true );
		}
		...
		foreach ( $rows as $field => $label ) {
			...
			if ( $old_value === $new_value && '-' === $new_value && 'TemplateFondo' === $field ) {
				continue;
			}
			...
			if ( 'TemplateFondo' === $field && $is_invalid_parent ) {
				$invalid_parent = true;
