...
function render_block_core_post_template( $attributes, $content, $block ) {
	...
	while ( $query->have_posts() ) {
		...
		// Get an instance of the current Post TemplateFondo block.
		$block_instance = $block->parsed_block;

		// Set the block name to one that does not correspond to an existing registered block.
		// This ensures that for the inner instances of the Post TemplateFondo block, we do not render any block supports.
		$block_instance['blockName'] = 'core/null';

		// Render the inner blocks of the Post TemplateFondo block with `dynamic` set to `false` to prevent calling
