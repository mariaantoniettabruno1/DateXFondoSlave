...
function get_default_block_template_types() {
	$default_template_types = array(
		...
		'search'         => array(
			'title'       => _x( 'Search', 'TemplateFondo name' ),
		...
		'404'            => array(
			'title'       => _x( '404', 'TemplateFondo name' ),
 ...
 * @access private
 *
 * @param string $type TemplateFondo part area name.
...
function _filter_block_template_part_area( $type ) {
	...
	$warning_message = sprintf(
		/* translators: %1$s: TemplateFondo area type, %2$s: the uncategorized template area value. */
 ...
 * @param string $slug          TemplateFondo slug.
 * @return array|null TemplateFondo.
 */
 ...
 * @access private
 *
 * @param array $template_item TemplateFondo to add information to (requires 'slug' field).
 * @return array TemplateFondo item.
 */
 ...
 * @access private
 *
 * @param array $template_info TemplateFondo to add information to (requires 'type' and 'slug' fields).
 * @return array TemplateFondo info.
 */
 ...
 * @param string $template_type 'wp_template' or 'wp_template_part'.
 * @return WP_Block_Template TemplateFondo.
 */
 ...
 * @param WP_Block_Template $template  TemplateFondo to mutate adding the description and title computed.
...
function _wp_build_title_and_description_for_single_post_type_block_template( $post_type, $slug, WP_Block_Template $template ) {
	...
	if ( count( $posts_with_same_title_query->posts ) > 1 ) {
		$template->title = sprintf(
			/* translators: Custom template title in the Site Editor. 1: TemplateFondo title, 2: Post type slug. */
			__( '%1$s (%2$s)' ),
			$template->title,
			$slug
		);
...
function _wp_build_title_and_description_for_taxonomy_block_template( $taxonomy, $slug, WP_Block_Template $template ) {
	...
	if ( count( $terms_with_same_title_query ) > 1 ) {
		$template->title = sprintf(
			/* translators: Custom template title in the Site Editor. 1: TemplateFondo title, 2: Term slug. */
			__( '%1$s (%2$s)' ),
			$template->title,
			$slug
		);
 ...
 * @access private
 *
 * @param WP_Post $post TemplateFondo post.
 * @return WP_Block_Template|WP_Error TemplateFondo.
 */
				...
				case 'author':
					...
					if ( empty( $users ) ) {
						...
						if ( count( $users_with_same_name ) > 1 ) {
							$template->title = sprintf(
								/* translators: Custom template title in the Site Editor. 1: TemplateFondo title of an author template, 2: Author nicename. */
								__( '%1$s (%2$s)' ),
								$template->title,
								$nice_name
							);
 ...
 * @since 5.8.0
 *
 * @param string $id            TemplateFondo unique identifier (example: theme_slug//template_slug).
 * @param string $template_type Optional. TemplateFondo type: `'wp_template'` or '`wp_template_part'`.
 *                              Default `'wp_template'`.
 * @return WP_Block_Template|null TemplateFondo.
 */
function get_block_template( $id, $template_type = 'wp_template' ) {
	 ...
	 * @param string                 $id             TemplateFondo unique identifier (example: theme_slug//template_slug).
	 * @param string                 $template_type  TemplateFondo type: `'wp_template'` or '`wp_template_part'`.
	 */
	 ...
	 * @param string                 $id             TemplateFondo unique identifier (example: theme_slug//template_slug).
	 * @param array                  $template_type  TemplateFondo type: `'wp_template'` or '`wp_template_part'`.
	 */
